...
class SFTTrainer(_BaseTrainer):
    ...
    def __init__(
        ...
        if not args.use_liger_kernel:  # liger supports dft loss by just passing use_token_scaling=True
            if args.loss_type == "nll":
                ...
                raise ValueError(
                    f"Invalid `loss_type` {args.loss_type} passed. Supported values are 'nll', 'dft' and 'eaft'."
                )
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.args.use_liger_kernel:
            ...
            inputs["use_eaft"] = self.args.loss_type == "eaft"
            inputs["eaft_alpha"] = self.args.eaft_alpha if self.args.loss_type == "eaft" else None
